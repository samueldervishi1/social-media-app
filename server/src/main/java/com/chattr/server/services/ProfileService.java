...
/**
 * Service responsible for user profile updates, password management, and account state toggling
 * (activate/deactivate/softly delete).
 */
@Service
...
public class ProfileService {
  ...
  public void softDeleteUser(String userId) {
    ...
    loggingService.logInfo(
        "ProfileService", "softDeleteUser", "User soft-deleted: userId " + userId);
  }

  public void deactivateUser(String userId) {
    toggleUserFlag(userId, false, true, LocalDateTime.now());
    loggingService.logInfo(
        "ProfileService", "deactivateUser", "User deactivated: userId " + userId);
  }
  ...
  public void blockUser(String blockerId, String targetId) {
    ...
    User blocker =
        userRepository
            .findById(blockerId)
            .orElseThrow(
                () -> new CustomException(404, String.format(Messages.USER_NOT_FOUND, blockerId)));

    User target =
        userRepository
            .findById(targetId)
            .orElseThrow(
                () -> new CustomException(404, String.format(Messages.USER_NOT_FOUND, targetId)));
  ...
  public User makePublic(String userId) {
    User user =
        userRepository
            .findById(userId)
            .orElseThrow(
                () -> new CustomException(404, String.format(USER_NOT_FOUND_BY_ID, userId)));
  ...
  private void toggleUserFlag(
      String userId, boolean deleted, boolean deactivated, LocalDateTime deactivationTime) {
    validateUserId(userId);
    User user = findUserById(userId);
  ...
  private User findUserById(String userId) {
    return userRepository
        .findById(userId)
        .orElseThrow(
            () -> new CustomException(404, String.format(Messages.USER_NOT_FOUND_BY_ID, userId)));
  }
