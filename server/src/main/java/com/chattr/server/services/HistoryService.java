...
/**
 * Service to manage conversation histories by session and user ID. Supports saving, fetching,
 * deleting, and auto-cleanup of old history.
 */
@Service
public class HistoryService {
  ...
  public History saveHistory(
      String sessionId, String userId, List<QuestionAnswerPair> questionAnswerPairs) {
    History history =
        historyRepository
            ...
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        String.format(Messages.NO_HISTORY_ERROR, sessionId)));
  ...
  public void deleteOldHistory() {
    ...
    if (!outdated.isEmpty()) {
      ...
      LOGGER.info(
          "Scheduled cleanup: Deleted {} outdated history records (before {})",
          outdated.size(),
          cutoffDate);
    } else {
