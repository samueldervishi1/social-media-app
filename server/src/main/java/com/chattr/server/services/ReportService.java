...
/**
 * Service responsible for handling user-generated post-reports, duplicate detection, and flagging
 * posts as reported.
 */
@Service
  ...
  public Report report(Report report) {
    try {
      ...
      loggingService.logInfo(
          "ReportService", "report", "User" + userId + "reported post" + postId + ".");
  ...
  private void markPostAsReported(String postId) {
    Post post =
        postRepository
            .findById(postId)
            .orElseThrow(
                () -> new CustomException(400, String.format(Messages.POST_NOT_FOUND, postId)));
