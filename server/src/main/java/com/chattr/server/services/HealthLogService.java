...
/**
 * Service for tracking periodic application health checks, storing logs by date, and avoiding
 * duplicate entries within 30 minutes.
 */
@Slf4j
@Service
      ...
      if (!logs.isEmpty()) {
        ...
        if (!entries.isEmpty()) {
          LocalDateTime lastTimestamp =
              LocalDateTime.parse(entries.get(entries.size() - 1).getTimestamp());
