...
public class PostController {
  ...
  public ResponseEntity<Post> getPost(@PathVariable String postId) {
    ...
    activityLogService.log(
        post.getUserId(), "POST_GET_BY_ID", "Retrieving post with ID: " + postId);
  ...
  public long getUserPostCount(@PathVariable String userId) {
    activityLogService.log(
        userId, "POST_COUNT_BY_USER", "Retrieving post count for user: " + userId);
  ...
  public ResponseEntity<Map<String, Object>> getAllPostsPaged(
    ...
    activityLogService.log(
        "anonymous", "POST_LIST_PAGED", "Paginated posts retrieved, page " + page);
  ...
  public ResponseEntity<?> getExplorePosts(@RequestParam(defaultValue = "10") int limit) {
    ...
    activityLogService.log(
        "anonymous", "EXPLORE_POSTS", "Explore tab fetched, top " + limit + " posts");
  ...
  public ResponseEntity<Integer> isPostLikedByUser(
