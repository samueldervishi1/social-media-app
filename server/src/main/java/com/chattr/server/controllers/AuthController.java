...
public class AuthController {
  ...
  public ResponseEntity<Error> register(
    ...
    try {
      ...
      for (Map<String, Object> query : queries) {
        ...
        if (rawQuery instanceof String str
            && str.contains("$.channelId=" + Messages.REQUIRED_CHANNEL_ID)) {
          channelValid = true;
        }
      }
      ...
      return ResponseEntity.ok(
          createResponse("200", Messages.REGISTER_SUCCESS, Messages.REGISTER_SUCCESS, "success"));
      ...
      activityLogService.log(
          "unknown", "REGISTER_ERROR", "Unexpected registration error: " + e.getMessage());
  ...
  private ResponseEntity<Error> createErrorResponse(CustomException e) {
    Error error =
        createResponse(String.valueOf(e.getCode()), e.getMessage(), e.getMessage(), "error");
  ...
  private Error createResponse(String code, String message, String reason, String status) {
    return new Error()
        ...
        .schemaLocation("/chattr/api/storm/provision/v1.3.2/auth")
        .type("auth_response");
  }
}
