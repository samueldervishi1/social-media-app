...
/**
 * Central exception handler for all REST controllers. Handles custom and generic exceptions and
...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<Error> handleCustomException(CustomException ex) {
    ...
    if (error == null) {
      error =
          buildBasicError(
              String.valueOf(ex.getCode()), ex.getMessage(), HttpStatus.valueOf(ex.getCode()));
    }
